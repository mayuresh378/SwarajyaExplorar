...
export default function FortList() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="section-title text-3xl">{t(ui.fortExplorer)}</h1>
        <p className="text-stone-600 mt-4">{t(ui.fortExplorerDesc)}</p>
      </div>

      {/* Filters */}
      <div className="bg-white rounded-xl shadow-sm border border-amber-100 p-4 mb-8">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-stone-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent text-sm"
            />
          </div>
          <div className="flex gap-3">
            <div className="relative">
              ...
              <select
                ...
                className="pl-10 pr-8 py-2.5 border border-stone-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm appearance-none bg-white"
              >
            ...
            <select
              ...
              className="px-4 py-2.5 border border-stone-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm appearance-none bg-white"
            >
      ...
      <p className="text-sm text-stone-500 mb-4">{filtered.length} {t(ui.fortsFound)}</p>

      {filtered.length === 0 ? (
        <div className="text-center py-16">
          <p className="text-stone-500 text-lg">{t(ui.noFortsFound)}</p>
          <button onClick={() => { setSearch(''); setDifficulty('all'); setDistrict('all'); }} className="mt-4 text-orange-700 font-medium hover:underline">
            {t(ui.clearFilters)}
          </button>
        </div>
      ) : (
